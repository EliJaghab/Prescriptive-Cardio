...
class MQTTViewController: UIViewController, MQTTSessionDelegate {
    ...
    @IBAction func startRunPressed(_ sender: AnyObject) {

        let heart_data = ["120", "125", "130", "135", "140", "147", "153", "158", "164", "170", "171", "170"]
        //"160", "152", "145", "140"]
        //"135", "130", "130", "130", "130", "170", "160", "148", "140", "135", "130", "130", "130", "130", "120",    "130","130","130","130","130","130","130","130", "130"]
        for row in heart_data {
            let channel = "heart_data"
            //let message = "{ \"time\":\"" + row[0] + "\", \"BPM\":\"" + row[1] + "\"}"
            let message = "{ \"BPM\":\"" + row + "\"}"

            //let data = message.data(using: .utf8)!
            self.mqttSession.publish(message.data(using: .utf8)!, in: channel, delivering: .atMostOnce, retain: false) { (error) in
                switch error {
                case .none:
                    self.appendStringToTextView("Published \(message) on channel \(channel)")
                default:
